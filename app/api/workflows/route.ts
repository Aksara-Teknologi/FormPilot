...
export async function GET(request: Request) {
  try { return Response.json({ scenarios: await listWorkflowScenarios(currentUser(request)) }); }
...
export async function POST(request: Request) {
  try {
    ...
    const input = body as Record<string, unknown>; const owner = currentUser(request);
